#from linkaform_api import mongo_util
import mongo_util
from pymongo import MongoClient
from bson.objectid import ObjectId



def search_ids(dbname, port, collection_name, folio_list):
    print 'looking on dbname ...' , dbname
    found_it = False
    cur_db = mongo_util.connect_mongodb(dbname, host, port)
    cur_col = mongo_util.get_mongo_collection(cur_db, collection_name )
    for folio in folio_list:
        query = {'_id':ObjectId(folio)}
        records = mongo_util.get_collection_objects(cur_col, query)
        if records.count() > 0:
            for res in records:
                print '-----------------------------------------'
                print ' folio:', res['folio']
                print 'id ', res['_id']
                print 'database', dbname
            found_it = True
    print 'next db ...'
    return found_it

def search_folios(dbname, port, collection_name, folio_list):
    print 'looking on dbname ...' , dbname
    found_it = False
    cur_db = mongo_util.connect_mongodb(dbname, host, port)
    cur_col = mongo_util.get_mongo_collection(cur_db, collection_name )
    for folio in folio_list:
        query = {'folio':folio}
        records = mongo_util.get_collection_objects(cur_col, query)
        if records.count() > 0:
            for res in records:
                print '-----------------------------------------'
                print ' folio:', res['folio']
                print 'id ', res['_id']
                print 'database', dbname
            found_it = True
    print 'next db ...',found_it
    return found_it


def search_workflows(dbname, port, collection_name, folio_list):
    print 'looking on dbname ...' , dbname
    found_it = False
    cur_db = mongo_util.connect_mongodb(dbname, host, port)
    cur_col = mongo_util.get_mongo_collection(cur_db, collection_name )
    for folio in folio_list:
        query = {'_id':ObjectId(folio)}
        records = mongo_util.get_collection_objects(cur_col, query)
        if records.count() > 0:
            for res in records:
                print '-----------------------------------------'
                print ' folio:', res['folio']
                print 'id ', res['_id']
                print 'database', dbname
            found_it = True
    print 'next db ...',found_it
    return found_it

def search_lost_folios(dbname, port, collection_name, form_list):
    print 'looking on dbname ...' , dbname
    found_it = {}
    infosync_db = mongo_util.connect_mongodb(dbname, host, port)
    infosync_cr = mongo_util.get_mongo_collection(infosync_db, 'infosync' )
    cur_db = mongo_util.connect_mongodb(dbname, host, port)
    cur_col = mongo_util.get_mongo_collection(cur_db, collection_name )
    for form_id in form_list:
        query = {'form_id':form_id}
        records = mongo_util.get_collection_objects(cur_col, query)
        for res in records:
                print '-----------------------------------------'
                print ' folio:', res['folio']
                version_ids = res['versiones']
                if version_ids:
                    old_folio = search_folio_in_versions(infosync_cr,version_ids, folio)
                if old_folio:
                    found_it.update({old_folio:res['folio']})
                    res['folio'] = old_folio
                    #search how to save
                    cur_col.save()
    print 'next db ...',found_it
    print 'found this folios', found_it
    return found_it

def search_in_versions(infosync_cr, version_ids, folio):
    for version_id in version_ids:
        #checar como hacer el in de object ids
        query = {'_id': {$in:ObjectId(version_ids), 'folio':{'$neq':folio}}
        #query = {'_id': {$in:ObjectId(version_id), 'folio':{'$neq':folio}}
        #revisar si se puede hacer un query a la version 1
        records = mongo_util.get_collection_objects(cur_col, query)
        if records.count() > 0:
            for res in records:
                print ' folio:', res['folio']
                return res['folio']
    return False

host='localhost'

port=27019
collection_name = 'form_answer'
collectoin_name = "workflow_data"
cr = MongoClient()
cr = MongoClient(host, port)

databases = ['infosync_answers_client_126','infosync_answers_client_9']
databases = cr.database_names()

folio_list = ['254010-96','239265-96',  '255619-96']
folio_list = [ '57111ed923d3fd3044bca122']
folio_list = ['53d6e61001a4de609b882a06']
folio_list = ['571a9cb823d3fd5bf31860c1', '571a9cd923d3fd5bf31860c4']
folio_list = [ '255619-96']
#pdf que no imprime la imagen
folio_list = ['571e6ea323d3fd2744ee502d', '571ea1cf23d3fd27400fbf0d']
found_all=0

worklfow = ["571f9ce723d3fd2ea8d0bd2b"]

folio_list = ['57111ed923d3fd3044bca122']

for dbname in databases:
    if dbname in ['test', 'infosync', 'admin']:
        continue
    found_it = search_workflows(dbname, port, collection_name, folio_list)
    if found_it:
        found_all += 1
        if found_all == len(folio_list):
            break

for dbname in databases:
    if dbname in ['test', 'infosync']:
        continue
    found_it = search_ids(dbname, port, collection_name, folio_list)
    if found_it:
        found_all += 1
        if found_all == len(folio_list):
            break
