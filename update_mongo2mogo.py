...
#prodction
dbname_prod = 'infosync_answers_client_96'
collection_name_prod = 'form_answer'
#port=27020
port=27017
host='localhost'
...
file_path ='/var/tmp/forma-contacto_2050.csv'
...
def update_answers_by_folio(dbname, host, port, collection_name ,file_path):
	#connectos to backup
	cr_db_back = connect_mongodb(dbname, host, port)
	cr_col_back = get_mongo_collection(cr_db_back, collection_name )
	#connects to prodcution
	cr_db_prod = connect_mongodb(dbname_prod, host_prod, port_prod)
	cr_col_prod = get_mongo_collection(cr_db_prod, collection_name_prod )
	all_folios = get_file_to_import(file_path)
	landings = get_landings_by_wrong_name()
	#all_folios = [{'folio': '123483-96'},{'folio': '55806-96'} ]
	count = 0
	for folio in all_folios:
		record_obj = cr_col_back.find(folio)
		record = record_obj.next()
		new_answers = record['answers']
		try:
			new_answers["5549395d01a4de0439331fd7"] = landings[new_answers["5549395d01a4de0439331fd7"]]
		except KeyError:
			pass
		cr_col_prod.update(folio, {"$set":{'answers':new_answers}})
		count += 1
		print 'udated', count


update_answers_by_folio(dbname, host, port, collection_name ,file_path)

def get_landings_by_wrong_name():
	landing_names = get_landing_relation()
	landings_wrong_names = {}
	for landing_name, values in landing_names.iteritems():
		for wrong_name in values:
			landings_wrong_names[wrong_name] = landing_name
	return landings_wrong_names
